...
export async function generateMetadata({
  ...
  try {
    ...
    const { data: animal, error: animalError } = await supabase
      .from("animals")
      .select(
        "id, animal_name, animal_type, breed, age_label, city, island, photo_url, is_adopted"
      )
      .eq("id", id)
    ...
    /*
     * Toutes les actions de partage manuel
     * utilisent maintenant le même visuel
     * que les publications automatiques Facebook.
     */
    const brandedShareImage =
      `${SITE_URL}/api/facebook/share-image/${encodeURIComponent(
        id
      )}?mode=${animal.is_adopted ? "adopted" : "available"}`;
